...
This vulnerability lies in the `/goform/switch_mode` page which influences the lastest version of Tenda Router AC11. (FYI, lastest version of this product is [AC11_V02.03.01.104_CN](https://www.tenda.com.cn/download/detail-3163.html))

## Vulnerability description

There is a stack buffer overflow vulnerability in function `tpi_wan_mode_check_update_info`, whose symbol name is inferred from the log function.

This function uses `strncpy` to copy the string pointed by `v4` of length `v6 + 1` into a stack buffer pointed by `v12`, while both `v4` and `v6` is retreived from a nvram variable `wan0_proto`.

![1](1.png)

the `wan0_proto` variable can be controlled by attacker by calling the function `sub_800C6448` which gets in a parameter called `wanType` and use that input value to set `wan0_proto` without any check.

![2](2.png)

So by first setting the `wan0_proto` and then requesting the page `/goform/switch_mode`, the attacker can easily perform a **Deny of Service Attack** or **Remote Code Execution** with carefully crafted overflow data.
...
```plain
POST /goform/switch_mode HTTP/1.1
Host: 192.168.0.1
Content-Length: 103
User-Agent: Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.66 Safari/537.36
Content-Type: application/x-www-form-urlencoded;
...
- 2021.05.02 CNVD ID assigned: CNVD-2021-32412
- 2021.07.19 Tenda claim to have fixed this issue
- 2021.10.12 CVE ID assigned: CVE-2021-32124
