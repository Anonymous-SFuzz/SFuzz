...
This function uses `strcpy` to copy the string pointed by `v5` into a stack buffer pointed by `v8`. `v5` is directly retrived from the nvram variable `led_time` without any security check.
...
So by first setting the `LEDCloseTime` and then requesting the page `/goform/updateLED`, the attacker can easily perform a **Deny of Service Attack** or **Remote Code Execution** with carefully crafted overflow data.
