...
def add_invert_rerun_simgr(pre_constrain, active_state_ori, other_supplement_memaddr = None, other_constrains = None, max_step_cnt = None, max_active_state_cnt = None):
    ...
    while len(simgr.active)>0:
        ...
        # if adr != None:
        #     print(simgr.one_active.memory.load(adr, 4))
        step_cnt += 1
        if (max_step_cnt and step_cnt>max_step_cnt):
            break
        if max_active_state_cnt and len(simgr.active)>max_active_state_cnt:
            pending_states = []
    ...
    l.info("No more active states, or failed to reach endaddr with limited steps, current simgr.stashes %r" % simgr.stashes)
    return True
...
def sink_hook(active_state):
    ...
    # if we only want one active state reaching sink addr, keep the following break
    # break   
